...
for cmd in docker poetry ffmpeg; do
...
export RUNWAY_API_MODE="${RUNWAY_API_MODE:-mock}"
...
echo "==> Running MVP smoke with provider mode ${RUNWAY_API_MODE}"
set +e
"$python_bin" scripts/e2e_mvp_smoke.py --repo-root "$repo_root" --api-base-url "$api_base_url" "$@"
status=$?
set -e

if [[ $status -ne 0 ]]; then
  echo
  echo "Smoke failed. API log tail (${api_log}):" >&2
