...
def test_overlay_qa_fails_on_extremely_wide_hook_text() -> None:
    ...
    assert report.verdict == QAVerdict.FAIL
    assert any(f.code == "overlay_text_did_not_fit" for f in report.findings)
    hookFinding = next(f for f in report.findings if f.code == "overlay_text_did_not_fit")
    assert cast(dict[str, Any], hookFinding.details).get("planned_role") == "hook"
